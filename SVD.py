...
# Create a directory to save the ROC plots
if not os.path.exists('roc_plots'):
    os.makedirs('roc_plots')

# Create a color map for plotting
colors = plt.cm.rainbow(np.linspace(0, 1, len(models)))

# Plot and save ROC curves for each model
for model, name, color in zip(models, model_names, colors):
    ...
    for train_idx, test_idx in kf.split(X_train):
        ...
        probas_ = model.predict_proba(X_test_kf)

        fpr, tpr, _ = roc_curve(y_test_kf, probas_[:, 1])
        tpr_list.append(np.interp(mean_fpr, fpr, tpr))

    mean_tpr = np.mean(tpr_list, axis=0)
    mean_auc = roc_auc_score(y_test_kf, probas_[:, 1])

    plt.plot(mean_fpr, mean_tpr, color=color, label=f'{name} (AUC = {mean_auc:.2f}')
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])

    # Save the ROC plot as an image file
    plt.savefig(f'roc_plots/{name}_ROC.png')
...
for model, name, color in zip(models, model_names, colors):
    ...
    # Save the ROC plot as an image file
    plt.savefig(f'roc_plots/{name}_ROC_2.png')
