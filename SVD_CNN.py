...
# Choisir une liste de valeurs de k à tester
k_values = [10, 20, 50, 100, 500]  # Vous pouvez ajuster cette liste selon vos besoins

# Initialiser une liste pour stocker les scores moyens de validation croisée
cv_scores = []

# Appliquer la validation croisée pour chaque valeur de k
for k in k_values:
    svd = TruncatedSVD(n_components=k, random_state=42)
    train_images_svd = svd.fit_transform(train_images)
    test_images_svd = svd.transform(test_images)
    
    # Récupérer le nombre de colonnes du DataFrame
    num_columns = train_images_svd.shape[1]
    ...
    num_samples = len(train_images_svd)
    images = train_images_svd.reshape(num_samples, m, n, -1)
    ...
    scores = cross_val_score(estimator=model, X=X_train_cnn, y=y_train, cv=5, scoring='accuracy')
